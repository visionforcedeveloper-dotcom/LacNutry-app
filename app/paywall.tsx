...
export default function PaywallScreen() {
  ...
  const initializeBilling = async () => {
    try {
      ...
      if (Platform.OS === "android") {
        ...
        console.log("💳 Google Play Billing Library configurada");
        console.log("📦 Product IDs configurados:");
        pricingPlans.forEach(plan => {
          console.log(`   - ${plan.productId}: ${plan.title}`);
        });
      } else if (Platform.OS === "ios") {
        console.log("📱 Plataforma: iOS");
        console.log("💳 StoreKit configurado");
      } else {
        console.log("🌐 Plataforma: Web - Modo simulação");
      }
  ...
  const handleSubscribe = async (planId: string) => {
    ...
    try {
      ...
      console.log(`💳 Iniciando compra do plano: ${plan.title}`);
      console.log(`📦 Product ID: ${plan.productId}`);
      console.log(`💰 Valor: R$ ${plan.price}`);
      
      if (Platform.OS === "android") {
        console.log("🤖 Processando via Google Play Billing...");
        await processPurchaseAndroid(plan);
      } else if (Platform.OS === "ios") {
        console.log("🍎 Processando via StoreKit...");
        await processPurchaseIOS(plan);
      } else {
        console.log("🌐 Modo simulação (Web)");
        await processPurchaseSimulation(plan);
      }
    } catch (error) {
  ...
  const processPurchaseAndroid = async (plan: PricingPlan) => {
    console.log("📱 [Android] Iniciando fluxo de compra");
    console.log("📱 [Android] Product ID:", plan.productId);
    
    await new Promise(resolve => setTimeout(resolve, 1500));
    
    console.log("✅ [Android] Compra simulada com sucesso");
    console.log("💡 Para produção, integre com: expo-in-app-purchases ou react-native-purchases");
    
    await completeSubscription();
    router.replace("/(tabs)");
  };

  const processPurchaseIOS = async (plan: PricingPlan) => {
    console.log("📱 [iOS] Iniciando fluxo de compra");
    console.log("📱 [iOS] Product ID:", plan.productId);
    
    await new Promise(resolve => setTimeout(resolve, 1500));
    
    console.log("✅ [iOS] Compra simulada com sucesso");
    console.log("💡 Para produção, integre com: expo-in-app-purchases ou react-native-purchases");
  ...
  const processPurchaseSimulation = async (plan: PricingPlan) => {
    console.log("🌐 [Web] Simulando compra");
    
    await new Promise(resolve => setTimeout(resolve, 1500));
    
    console.log("✅ [Web] Compra simulada com sucesso");
